...
const GeneralBriefForm = dynamic(() => import("@/components/GeneralBriefForm"));
...
export async function generateMetadata({
  ...
  const t = await getTranslations({ locale, namespace: "generalBrief" });
  const canonicalUrl = `${BASE_URL}/${locale}/brief/general`;
...
export default async function GeneralBriefPage({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
  ...
  const t = await getTranslations({ locale, namespace: "generalBrief" });
  const tBrief = await getTranslations({ locale, namespace: "brief" });
      ...
      <main id="main-content" tabIndex={-1} className="bg-[#F8FAFC]">
        ...
        <div className="bg-[#0F172A] pt-32 pb-16">
          <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
            <BackLink href="/brief">{tBrief("backToOptions")}</BackLink>
        ...
        <AudioBriefSection namespace="generalBrief" />
      </main>
      <Footer />
    </>
  );
