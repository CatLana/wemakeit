...
const SLUG = "solutions/for-startups";
...
export async function generateMetadata({
  ...
  const t = await getTranslations({
    locale,
    namespace: "solutionsStartupsPage.metadata",
  });
...
export default async function StartupsPage({
  ...
  const t = await getTranslations({
    locale,
    namespace: "solutionsStartupsPage",
  });
  const services = t.raw("services") as Array<{
    name: string;
    detail: string;
  }>;
  const whyPoints = t.raw("why") as string[];

  const jsonLd = {
    "@context": "https://schema.org",
    "@type": "Service",
    name: "Startup MVP and Product Development Services",
          ...
          <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <ul role="list" className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {services.map((service) => (
                <li
                  key={service.name}
                  className="rounded-xl border border-slate-200 p-5 bg-[#F8FAFC]"
                >
          ...
          <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <ul role="list" className="space-y-4">
              {whyPoints.map((point, i) => (
                ...
                >
                  <div className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-[#22D3EE]/15">
                    <svg
