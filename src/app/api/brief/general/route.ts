...
const FIELD_LABELS: Record<keyof Omit<BriefData, "name" | "email">, string> = {
  business: "Tell us about your business, what you do and who you serve.",
  challenge: "What's the main challenge or opportunity you'd like to discuss?",
  triedSoFar: "What have you already tried, or what's currently in place, if anything?",
  successLooksLike: "What would a great outcome look like for you?",
      ...
      <table width="600" cellPadding="0" cellSpacing="0" style="background:#fff;border-radius:12px;overflow:hidden;border:1px solid #e2e8f0;">
        <tr>
          <td style="background:#0f172a;padding:28px 32px;">
            ...
            <p style="margin:8px 0 0;font-size:13px;color:#94a3b8;">New general brief from wemakeit.ie</p>
          </td>
        </tr>
        <tr>
        ...
        <tr>
          <td style="padding:0 32px 24px;">
            <p style="font-size:12px;color:#94a3b8;margin:0;">This email was sent from the general brief form at wemakeit.ie</p>
          </td>
        </tr>
      </table>
    </td></tr>
...
export async function POST(req: NextRequest) {
  ...
  const subject = data.name ? `General brief from ${data.name}` : "New general brief";
  ...
  if (error) {
    console.error("Resend error (general brief):", error);
