...
function buildCustomerHtml(data: z.infer<typeof schema>): string {
  return `<!DOCTYPE html>
<html lang="en">
      ...
      <table width="600" cellPadding="0" cellSpacing="0" style="background:#fff;border-radius:12px;overflow:hidden;border:1px solid #e2e8f0;">
        ...
        <tr>
          <td style="padding:32px;">
            <p style="margin:0 0 16px;font-size:16px;font-weight:700;color:#0f172a;">Thanks, ${data.name}</p>
            <p style="margin:0 0 20px;font-size:14px;color:#334155;line-height:1.6;">We've had a look at what you told us. Pick a time that suits you below and we'll see you on the call.</p>
            <a href="${CONSULTATION_BOOKING_URL}"
    ...
    <tr><td align="center">
      <table width="600" cellPadding="0" cellSpacing="0" style="background:#fff;border-radius:12px;overflow:hidden;border:1px solid #e2e8f0;">
        <tr>
          <td style="background:#0f172a;padding:28px 32px;">
            <span style="font-size:22px;font-weight:800;color:#fff;">We Make <span style="color:#22d3ee;">IT</span></span>
            <p style="margin:8px 0 0;font-size:13px;color:#94a3b8;">New discovery call request from wemakeit.ie</p>
          </td>
        </tr>
        <tr>
          <td style="padding:24px 32px 32px;">
            <p style="margin:0 0 16px;font-size:16px;font-weight:700;color:#0f172a;">
              Discovery Call Request: ${data.service}
            </p>
        ...
        <tr>
          <td style="padding:0 32px 32px;">
            <a href="mailto:${data.email}?subject=Re: Discovery Call: ${data.service}&body=Hi ${data.name},%0D%0A%0D%0AThank you for requesting a discovery call. I'd love to learn more about your project.%0D%0A%0D%0AKind regards,%0D%0ALana%0D%0AWe Make IT"
               style="display:inline-block;background:#22d3ee;color:#0f172a;font-weight:700;font-size:14px;padding:12px 24px;border-radius:10px;text-decoration:none;">
              Reply to ${data.name}
            </a>
          </td>
...
export async function POST(req: NextRequest) {
  ...
  const { error } = await resend.emails.send({
    ...
    to: RECIPIENT,
    replyTo: data.email,
    subject: `[wemakeit.ie] Discovery call: ${data.service} from ${data.name}`,
    html: buildHtml(data),
  });

  if (error) {
    console.error("Resend error (consultation internal notification):", JSON.stringify(error));
    return NextResponse.json({ error: "Failed to send email." }, { status: 500 });
  }

  // Best-effort: the internal notification above is the one that must succeed.
  // If the customer confirmation fails to send, we still return success, since
  // the enquiry has been received and Lana can follow up manually.
  try {
    const { error: customerEmailError } = await resend.emails.send({
      from: "We Make IT <onboarding@resend.dev>",
      to: [data.email],
      subject: "Pick a time for your discovery call | We Make IT",
      html: buildCustomerHtml(data),
    });
    if (customerEmailError) {
      console.error("Resend error (consultation customer email):", customerEmailError);
    }
  } catch (err) {
    console.error("Resend threw (consultation customer email):", err);
  }
